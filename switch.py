            ...
            for (ex, ey, ew, eh) in eyes:
                ...
                try:
                    ...
                    if os.path.exists(eye_filename):
                        print(f"Successfully saved {eye_filename}")
                    else:
                        print(f"Failed to save {eye_filename}")
...
def main():
    ...
    choice = simpledialog.askstring("Input", "Do you want to use the virtual mouse or physical mouse? (v/p):").strip().lower()
    root.update()  # Ensure the dialog is processed
    root.destroy()  # Destroy the root window after getting the input

    if choice == 'v':
        virtual_mouse()
    elif choice == 'p':
        print("Using physical mouse.")
    else:
        print("Invalid choice. Please restart the script and select 'v' for virtual mouse or 'p' for physical mouse.")
